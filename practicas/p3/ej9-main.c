	...
	do{
		...
		switch(opcion){
			...
			case 2:
				...
				scanf("%f",&auxiliar.unidades);
				getchar();

				if(introduceLibro(auxiliar)==1){
					printf("\nLibro añadido al registro");
				}
				else{
					printf("\nError añadiendo el libro al registro");
				}
	
			break;

			case 3:
				printf("\nExisten un total de %d libros registrados en el stock",cuentaRegistros());
			break;

			case 4:
				printf("\nListado de libros registrados en el stock:\n");

				elementos=cuentaRegistros();		
